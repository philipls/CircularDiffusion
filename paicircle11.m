function [ll,qaic,qbic,Pred,Gstuff] = paicircle11(Pvar, Pfix, Sel, Data, trace)
% ==========================================================================
...
% ===========================================================================
   ...
   tau2 = Overdispersion(4); % S1

   % Number of trials in each discriminability condition. (S1 a bit unbalanced).
   nlow = length(Data{1});
   nmed = length(Data{2});
   nhi = length(Data{3});
   ...
   % -----------------------------------------------------------------------------------------
   %    v1a...v3b,     eta1a....eta3b,    a,   Ter,  b1...b3, alpha, a1..a3,    sa     pi1]
   % ---------------------------------------------------- ------------------------------------

    Ub= [ 7.0*ones(1,6),  4.0*ones(1,6),  5.0,  1.0,  5.0*ones(1,4),  2*pi*ones(1,3),  2*a, 1.0]; 
    Lb= [-7.0*ones(1,6),  0.0*ones(1,6),  0.5,  0.1,    0*ones(1,4)  0*ones(1,3), 0, 0 ];
    Pub=[ 6.5*ones(1,6),  3.5*ones(1,6),  4.8,  0.8   4.5*ones(1,4)  (2*pi-eps)*ones(1,3), 2*a - eps, .95];
    Plb=[-6.5*ones(1,6),  0.0*ones(1,6),  0.7,  0.15  0.02*ones(1,4)  eps*ones(1,3), 0, .02];
    Pred = cell(3,3);
    if any(P - Ub > 0) | any(Lb - P > 0)
     ...
     %pen = [tau2, sum(Sel), log(nlow + nmed + nhi), sum(Sel)*log(nlow + nmed + nhi)]
...
% ===============================================================================================
   ...
   nw1 = nw + 1;
   nv1 = nv + 1;
   BiasAngle = sort(signed_angle(RawBias)); 
   ...
   tmax = 4.0;
   ter = Pj(10); % Have sa on the end now.
   ld = size(Dataj);
   if ld(2) ~= 3
   ...
   [t, gt, thetas, theta, ptheta, mtheta] = vjoint5density([Pj(1:9), Pj(11:13)], BiasAngle, tmax, nw1, nv1, nt);
   % Filter zeros  
